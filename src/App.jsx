...
function App() {
  ...
  const slideTransitionTimer = useRef(null);
  const classPointerType = useRef('mouse');
  ...
  const go = (index) => {
    ...
    window.clearTimeout(slideTransitionTimer.current);
    setSlideMotion(next > active ? 'forward' : 'back');
    setLeaving(active);
    setActive(next);
    slideTransitionTimer.current = window.setTimeout(() => setLeaving(null), 520);
  };

  const sectionClass = (name, index) => {
    if (active === index) return `${name} presentation-section section-active slide-motion-${slideMotion}`;
    if (leaving === index) return `${name} presentation-section section-leaving slide-motion-${slideMotion}`;
    return `${name} presentation-section`;
  };
  ...
  const chooseSurfaceClass = (id, event) => {
    ...
    setSelectedClass(id);
    setHoveredClass(null);
    if (classPointerType.current === 'mouse') startClassPreview(id, event);
  };
  ...
  useEffect(() => () => window.clearTimeout(slideTransitionTimer.current), []);
  ...
  return (
    <main onTouchStart={startSwipe} onTouchEnd={finishSwipe}>
      ...
      <section id="fluxo" className={sectionClass('flow-section', 1)}>
      ...
      <section id="condicoes" className={sectionClass('conditions-section', 2)}>
      ...
      <section id="matriz" className={sectionClass('matrix-section', 4)}>
      ...
      <section id="atencao" className={sectionClass('attention-section', 5)}>
      ...
      <section id="exemplos" className={sectionClass('examples-section', 6)}>
