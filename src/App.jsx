...
const nav = ['Início', 'Fluxo', 'Condições', 'Classes', 'Matriz', 'Atenção', 'Exemplos'];

function Status({ value }) {
  const labels = { '✓': 'Pode liberar', '!': 'Avaliar', '✕': 'Reprovar', '—': 'Consultar norma' };
  return <span className={`status status-${value}`} title={labels[value]} aria-label={labels[value]}>{value}</span>;
}
  ...
  return (
    <main>
      <nav className="topbar" aria-label="Navegação da apresentação">
        <button className="brand" onClick={() => go(0)} aria-label="Voltar ao início"><span>V</span> GUIA DE QUALIDADE</button>
        <div className="nav-links">
          {nav.map((item, index) => <button key={item} className={active === index ? 'active' : ''} onClick={() => go(index)}>{item}</button>)}
        </div>
        <span className="slide-counter">{String(active + 1).padStart(2, '0')} / 07</span>
      </nav>

      <section id="inicio" className="hero presentation-section">
        ...
        <div className="hero-content">
          ...
          <button className="primary-button" onClick={() => go(1)}>Iniciar guia <b>↓</b></button>
        </div>
        <FooterRule />
      </section>

      <section id="fluxo" className="presentation-section flow-section">
        ...
        <div className="flow-layout">
          ...
          <aside className="critical-note"><span>!</span><div><h3>Critérios que não podem ser esquecidos</h3><p><strong>Quantidade:</strong> verifique o nível do defeito e sua visibilidade conforme a classe.</p><p><strong>Função:</strong> não liberar se prejudicar montagem, função ou gerar corrosão.</p></div></aside>
        </div>
      ...
      <section id="condicoes" className="presentation-section conditions-section">
        ...
        <div className="conditions-layout">
          <div className="condition-list">
            <div><b>✓</b><p><strong>Superfície limpa</strong><br />ITS 176: superfícies limpas. ITS 177: recém-pintadas.</p></div>
            <div><b>1500–2000</b><p><strong>Iluminação</strong><br />Lux recomendados no local de inspeção.</p></div>
            <div><b>☁</b><p><strong>Luz difusa</strong><br />Iluminação uniforme, sem direção marcada.</p></div>
            <div><b>90°</b><p><strong>Ângulo</strong><br />Avaliar preferencialmente em ângulo reto à superfície.</p></div>
            <div><b>↗</b><p><strong>Visão lateral</strong><br />Pode avaliar ligeiramente de lado para cobrir ângulos.</p></div>
            <div><b>1m</b><p><strong>Distância</strong><br />Avaliar a 1 m de distância.</p></div>
          </div>
      ...
      <section id="classes" className="presentation-section classes-section">
      ...
      <section id="matriz" className="presentation-section matrix-section">
      ...
      <section id="atencao" className="presentation-section attention-section">
        ...
        <div className="attention-grid">
          ...
          <article className="attention-card"><span className="card-index">03</span><h3>Falhas e resíduos</h3><p>Verificar se contém falhas de pintura e resíduos na peça, como sujeira.</p></article>
          <article className="attention-card"><span className="card-index">04</span><h3>Forno e camadas</h3><p>Verificar a temperatura e a velocidade do forno, além do nível de camadas, para evitar desplacamento.</p></article>
          <article className="attention-card"><span className="card-index">05</span><h3>Cor e brilho</h3><p>Verificar desvios de cor e brilho, bem como respingos na superfície.</p></article>
        </div>
      ...
      <section id="exemplos" className="presentation-section examples-section">
        ...
        <div className="gallery"><figure><img src="/assets/paint-failure.jpeg" alt="Falha de pintura em peça metálica" /><figcaption><span>✕</span> FALHA DE PINTURA</figcaption></figure><figure><img src="/assets/rust.jpg" alt="Ferrugem em peça metálica pintada" /><figcaption><span>✕</span> FERRUGEM</figcaption></figure><figure><img src="/assets/contamination.jpg" alt="Contaminação visível em superfície" /><figcaption><span>✕</span> CONTAMINAÇÃO</figcaption></figure></div>
        <div className="final-call"><p>Na dúvida, não liberar automaticamente.</p><button className="primary-button" onClick={() => go(1)}>Revisar fluxo <b>↑</b></button></div>
        <FooterRule />
      </section>

      <div className="floating-controls" aria-label="Controles de slide"><button onClick={() => go(active - 1)} disabled={active === 0} aria-label="Seção anterior">←</button><button onClick={() => go(active + 1)} disabled={active === sections.length - 1} aria-label="Próxima seção">→</button></div>
    </main>
  );
}
